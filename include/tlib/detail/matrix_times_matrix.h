...
using mtm_row_tr1 = mtm < cblas_row, cblas_tr,   cblas_notr >;
using mtm_row_tr2 = mtm < cblas_row, cblas_notr, cblas_tr   >;
using mtm_row     = mtm < cblas_row, cblas_notr, cblas_notr >;

using mtm_col_tr1 = mtm < cblas_col, cblas_tr  , cblas_notr >;
using mtm_col_tr2 = mtm < cblas_col, cblas_notr, cblas_tr   >;
using mtm_col     = mtm < cblas_col, cblas_notr, cblas_notr >;
...
using mtv_row = mtv<cblas_row>;
using mtv_col = mtv<cblas_col>;
...
inline void mtm_row(
			...
			value_t const*const a, size_t const*const na,     size_t const*const /*wa*/, size_t const*const pia,
			value_t const*const b, size_t const*const nb, // is a row-major dense matrix
			value_t      *const c, size_t const*const /*nc*/, size_t const*const /*wc*/, size_t const*const /*pic*/
			)
{
  ...
  auto n  = num_elements(na,p) / nq;
	                                           ...
	                                           // A,x,y, m,  n,  lda
	     if(is_case_rm<1>(p,q,pia)) mtv_row     (b,a,c, mq, n1, n1  );
                                             // a,b,c  m,  n,  k,  lda,ldb,ldc    	     
	else if(is_case_rm<2>(p,q,pia)) mtm_row_tr1 (a,b,c, n2, mq, n1,  n1, mq, mq); // q=1 | A,C = CM | B = RM
	else if(is_case_rm<3>(p,q,pia)) mtm_row     (b,a,c, mq, n1, n2,  n2, n1, n1); // q=2 | A,C = CM | B = RM
	
	else if(is_case_rm<4>(p,q,pia)) mtm_row     (b,a,c, mq, n2, n1,  n1, n2, n2); // q=1 | A,C = RM | B = RM
	else if(is_case_rm<5>(p,q,pia)) mtm_row_tr2 (a,b,c, n1, mq, n2,  n2, mq, mq); // q=2 | A,C = RM | B = RM
	
	else if(is_case_rm<6>(p,q,pia)) mtm_row_tr2 (a,b,c,  n, nq, nq,  nq, mq, mq); // q=pi(1) | A,C = RM | B = RM
	else if(is_case_rm<7>(p,q,pia)) mtm_row     (b,a,c, mq,  n, nq,  nq,  n,  n); // q=pi(p) | A,C = RM | B = RM
