    ...
    py::array_t<T> const& b)
{
  ...
  auto pia = tlib::ttm::detail::generate_k_order_layout(p, p);
  auto pib = tlib::ttm::detail::generate_k_order_layout(2ul, 2ul);
  ...
  if(pb!=2)
    throw std::invalid_argument("Error calling ttmpy::ttm: second input should be a mtrix with order equal to 2.");
      ...
      auto wc  = tlib::ttm::detail::generate_strides(nc,pic);
  ...
  // auto nnc          = std::size_t(cinfo.size);
      ...
      tlib::ttm::ttm<T>(tlib::ttm::parallel_policy::combined, tlib::ttm::slicing_policy::combined, tlib::ttm::fusion_policy::all, 
                        q, p, 
